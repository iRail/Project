...
foreach($projects as $project){
    ...
    foreach($project['projects'] as $subproject){
        if(!empty($subproject['github'])){
            $repo_uri = GITHUBAPI . $subproject['github'];

            // Set URL to download
            curl_setopt($ch, CURLOPT_URL, $repo_uri);
            ...
            if($http_status == 200){
                ...
                $results = $db->query('SELECT id FROM endpoint WHERE uri="' . $repo_uri . '"');
                ...
                if(!empty($results['id'])){
                    ...
                    $stmt->bindValue(':uri', $repo_uri, SQLITE3_TEXT);
                    $stmt->bindValue(':data', $repo_data, SQLITE3_TEXT);
                    $stmt->execute();
                }
